...
test.describe("video background accessibility", () => {
    test("should not have accessibility issues with video background", async ({page}, testInfo) => {
        test.setTimeout(90000); // Give video extra time

        await page.goto("./samples/variations-video.html", {waitUntil: "networkidle"});

        // Wait for components and video
        await page.waitForFunction(() => {
            const heroes = document.querySelectorAll('ilw-hero');
            const videos = document.querySelectorAll('video');
            return heroes.length > 0 &&
                Array.from(heroes).every(hero => hero.shadowRoot !== null) &&
                videos.length > 0;
        }, { timeout: 30000 });
        ...
        if (accessibilityScanResults.violations.length > 0) {
            console.log('\n==========================================');
            console.log(`FOUND ${accessibilityScanResults.violations.length} ACCESSIBILITY VIOLATIONS (VIDEO)`);
            console.log('==========================================\n');
        ...
        if (!result) {
            throw new Error(
                `Found ${accessibilityScanResults.violations.length} accessibility violation(s) with video background. ` +
                `Check the console output above for details, or see the attached axe.html report.`
            );
        }
